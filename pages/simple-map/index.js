const { addMarkersCtxCases } = require('./context/addMarkers');
const { moveToLocationCtxCases } = require('./context/moveToLocation');
const { removeMarkersCtxCases } = require('./context/removeMarkers');
const { circleCases } = require('./data/circles');
const { includePointsCases } = require('./data/includePoint');
const { includePointModifyCases } = require('./data/includePointModify');
...
const { showLocationCases } = require('./data/showLocation');
...
const data = {
  ...
  showLocationCases,
  includePointsCases,
  includePointModifyCases,
  markersCases,
  polylineCases,
  circleCases,
  addMarkersCtxCases,
  removeMarkersCtxCases,
  moveToLocationCtxCases,
  // 这里定义需要录入到对象的属性字段
...
Page({
  ...
  // 选中后触发事件
  bindMapEventPickerChange: function (e) {
    const {
      currentTarget: {
        dataset: { targetArr }
      },
    ...
    const { data } = this;
    console.log('激活用例', data[targetArr][value].title, data[targetArr][value]);
    data[targetArr][value].trigger(this, ma);
  }
});
